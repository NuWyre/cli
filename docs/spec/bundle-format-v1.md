...
**Last revised:** 2026-05-26 (v1.0.18; v1-side documentation-vs-implementation reconciliation — anchor-repo `root.json` path layout corrected to the per-bundle_type 4-level form the reference verifier + production cron already use; see Revision history. v2 path unchanged: v2.0.1.)
...
- **2026-05-26 (v1.0.18)** — **Documentation-vs-implementation reconciliation amendment** (Phase 5 anchor-bootstrap punch list; third-dot additive per SPEC_GOVERNANCE.md §3.1). Corrects the anchor-repo `root.json` path layout in §3 (path discipline), §4 (manifest `github` example), §12.1, §12.2, and §12.4 from `daily-roots/<organization_id>/<date>/root.json` to **`daily-roots/<organization_id>/<date>/<bundle_type>/root.json`** — the per-bundle_type subdirectory the production daily-root cron has staged at since Phase 6.2.B-F session 69 (BACKLOG 1.29) and the reference verifier has fetched since Phase 6.2.C session 70 (BACKLOG 1.33). **The spec text had drifted behind the reference implementation**: `apps/cli/internal/checks/github_fetch.go` `FetchRootJson`, `check7_github.go`, and `apps/api/src/lib/daily-root/act.ts` all construct the 4-level path; spec §12 still described the pre-session-70 3-level path. A third-party verifier built from spec §12.4 alone would construct the 3-level path — which no NuWyre anchor will ever live at, so it could never succeed against the anchor repo (today: zero anchors exist; post-go-live: a guaranteed 404 while the real `root.json` sits one directory deeper) — a NOT-IMPLEMENTABLE-from-spec-alone gap (documentation-vs-implementation drift sub-pattern, continued). The `<bundle_type>` segment (closed §4.1 vocabulary `{customer-export, audit-log-export, example-demo, sandbox-preview}`) isolates a single (organization_id, date) tuple's customer-export anchor from its audit-log-export anchor so the two artifact sets don't collide on one `root.json` — the same per-(organization_id, root_date, bundle_type) isolation v1.0.15 §8.2 pinned at the `daily_roots` table layer. **Reference-implementation reconciliation also lands** at the v1 TS writers (`generate-bundle.ts` + `generate-audit-log-bundle.ts` `github_anchors[].path` field — informational provenance only; check 7 reconstructs the fetch path from `organization_id` + `date` + `bundle_type` and does not read the `path` field), the example-bundle anchor-bootstrap scripts (`prepare-anchor.ts` staging layout + `finalize-anchor.ts` `composeAnchorReal` path arg), `anchor-schema.ts` doc + diagnostic text, and the verifier-package doc comments (`anchor_repo_schema.go`, `doc.go`, `github-anchor.ts`). **Sibling fix (same punch list)**: `finalize-anchor.ts`'s `manifest.anchor_status.github_status` corrected from the non-canonical literal `"confirmed"` to a passthrough of `github_anchors/<date>.json:mirror_status` (= `"anchored"`); `"confirmed"` is outside the §4.2 + §11.1 canonical 4-state enum (`not_attempted | anchor-pending | anchored | failed`) and would have failed check 7's `validateGitHubConsistency` on both the enum-membership and per-day-agreement tests. **Backward compatibility**: no on-wire bundle byte change — the `<bundle_type>` segment is anchor-repo directory layout, not bundle content. Zero anchored bundles exist (the anchor pipeline is in deploy-bootstrap state; pre-customer-#1), so there is no migration; the correction makes the spec match the only implementation that has ever run. **Six tenants framing**: T1 (long-term correctness) load-bearing — the per-bundle_type isolation is a real requirement v1.0.15 already pinned at the DB layer; documenting it at the anchor-repo layer closes the cross-layer gap before customer-#1's first anchor lands. T4 (simplicity) — single source of truth: spec, verifier, cron, and TS writers now describe one path. T5 (customer trust) — a compliance buyer's verifier built from the published spec now resolves the path NuWyre's anchors actually live at. **External-implementer simulation**: a Python verifier built from §12.4 alone now reaches the correct 4-level path. **Reviewer-pass closure pre-commit**: spec-conformance-reviewer.
...
**Path discipline.** Bundle artifact paths in `manifest.json:artifacts[].path` are bundle-relative (`events.jsonl`, `rfc3161_receipts/2026-04-22__freetsa.tsr`). The v3.1.8 anchor-repo `root.json` schema uses bare filenames inside a `daily-roots/<organization_id>/<date>/<bundle_type>/` directory in the anchors repo (`2026-04-22.ots`, `2026-04-22__freetsa.tsr`); the two layouts are intentionally distinct. See `packages/evidence/src/anchor-schema.ts` for the anchor-repo schema and §12 below.
...
```jsonc
{
  ...
  "anchors": {
    ...
    "github": {
      ...
      "path":            null,                          // e.g., "daily-roots/<organization_id>/2026-04-22/<bundle_type>/root.json" when anchored
      "anchored_at":     null,
...
- `commit_sha` is the hex SHA of the SSH-signed commit in the anchor repo whose tree contains `daily-roots/<organization_id>/<date>/<bundle_type>/root.json`. Length matches `commit_sha_format`: 40-char lowercase hex for sha1, 64-char lowercase hex for sha256.
- `path` is the repo-relative path to that file (`"daily-roots/<organization_id>/<date>/<bundle_type>/root.json"` per Phase 4 prereq Session A's per-org extension + the Phase 6.2.C session 70 per-bundle_type extension; see §12.2).
...
### 12.2 Anchor repo `root.json` schema (v3.1.8 + Phase 4 prereq per-org extension + Phase 6.2.C per-bundle_type extension)

The file at `daily-roots/<organization_id>/<date>/<bundle_type>/root.json` in the anchor repo follows the v3.1.8 schema with the Phase 4 prereq per-org extension and the Phase 6.2.C session 70 per-bundle_type extension (see `packages/evidence/src/anchor-schema.ts`):
...
**Phase 4 prereq per-org extension.** Pre-Path-A V1 used a platform-aggregate root (one root per UTC date across all customers' events); the anchor repo path was `daily-roots/<date>/root.json`. This contradicted bundle-export verifiability under multi-tenant production: the platform-anchored receipts couldn't be reproduced from one org's events, so Phase 4 CLI checks 5/6 would fail by design. Path A migration (`packages/database/supabase/migrations/20260509200000_daily_roots_per_org.sql`) adopted per-(organization_id, root_date) rows. The anchor repo path adds the `<organization_id>` segment; root.json carries the `organization_id` field; receipt_path / chain_path stay as bare filenames inside the per-(org, date, bundle_type) directory.

**Phase 6.2.C per-bundle_type extension (session 70, BACKLOG 1.33).** The anchor repo path adds a trailing `<bundle_type>` segment: `daily-roots/<organization_id>/<date>/<bundle_type>/root.json`. `<bundle_type>` is the bundle's `manifest.json:bundle_type` from the closed §4.1 vocabulary `{customer-export, audit-log-export, example-demo, sandbox-preview}`. The segment isolates a single (organization_id, date) tuple's customer-export anchor from its audit-log-export anchor so the two artifact sets do not collide on one `root.json` — the same per-(organization_id, root_date, bundle_type) isolation v1.0.15 §8.2 pinned at the `daily_roots` table layer. The reference verifier (`apps/cli/internal/checks/github_fetch.go` `FetchRootJson`) reconstructs this path from `manifest.organization_id` + the daily root's `date` + `manifest.bundle_type`; the production daily-root cron (`apps/api/src/lib/daily-root/act.ts`) stages anchor files at the identical path. The `<bundle_type>` value is validated against the closed vocabulary before URL construction (path-traversal defense, parallel to the `<organization_id>` UUID validation below).
...
2. Confirm `daily-roots/<organization_id>/<date>/<bundle_type>/root.json` exists and parses per the v3.1.8 schema. The `<organization_id>` segment is the bundle's `manifest.json:organization_id` and the `<bundle_type>` segment is the bundle's `manifest.json:bundle_type` (closed §4.1 vocabulary); an organization_id mismatch is a verification failure (a bundle's organization can't unilaterally read another org's anchor file), and the bundle_type segment selects this bundle type's anchor tree so a customer-export and an audit-log-export for the same (org, date) resolve to distinct `root.json` files.
