...
- `docs/spec/bundle-format-v1.md` — the bundle format specification (currently at **v1.0.18 (v1 path) + v2.0.1 (v2 path)**; v2.0.0 final promoted from v2.0.0-rc1 at Phase 7.F.4 session 102; v2.0.1 additive amendment at Phase 7.F.5 session 103 added §18.3.1 library-API-shape registry — pure informative, no on-wire byte change; v1.0.18 additive amendment 2026-05-26 reconciled the anchor-repo `root.json` path layout to the per-bundle_type 4-level form the reference verifier + production cron already use — documentation-vs-implementation drift closure, no on-wire byte change; 27/27 conformance fixtures (14 v1 + 13 v2) shipped at `docs/spec/fixtures/bundle-format-v1/`; CI gating extended to all 27 + KAT vectors at Phase 7.F.7 session 104)
